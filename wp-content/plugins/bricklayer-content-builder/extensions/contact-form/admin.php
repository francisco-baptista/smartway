...
function grunion_post_type_columns_filter( $cols ) {
	$cols = array(
		...
		'feedback_from'		=> __( 'From', THEME_TEXT_DOMAIN ),
		'feedback_message'		=> __( 'Message', THEME_TEXT_DOMAIN ),
		'feedback_date'			=> __( 'Date', THEME_TEXT_DOMAIN )
	);
	...
	switch ( $col ) {
		...
		case 'feedback_message':
			...
			if ( $post->post_status == 'trash' ) {
				...
				echo '"><a title="';
				echo esc_attr__( 'Resore this item from the Trash', THEME_TEXT_DOMAIN );
				...
				echo '">' . __( 'Restore', THEME_TEXT_DOMAIN ) . '</a></span> | ';

				echo "<span class='delete'> <a class='submitdelete' title='";
				echo esc_attr( __( 'Delete this item permanently', THEME_TEXT_DOMAIN ) );
				echo "' href='" . get_delete_post_link( $post->ID, '', true );
				echo "'>" . __( 'Delete Permanently', THEME_TEXT_DOMAIN ) . "</a></span>";
?>
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as spam', THEME_TEXT_DOMAIN );
				...
				echo '">';
				echo '<a class="submitdelete" title="' . esc_attr__( 'Trash', THEME_TEXT_DOMAIN );
				echo '" href="' . get_delete_post_link( $post_id );
				echo '">' . __( 'Trash', THEME_TEXT_DOMAIN ) . '</a></span>';
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as NOT spam', THEME_TEXT_DOMAIN );
...
<?php
		...
		case 'feedback_date':
			echo get_the_date( __( 'Y-m-d @ g:i:s A', THEME_TEXT_DOMAIN ) );
			break;
	}
}
...
function grunion_ajax_spam() {
	...
	if ( !current_user_can("edit_page", $post_id) )
		wp_die( __( 'You are not allowed to manage this item.', THEME_TEXT_DOMAIN ) );
	...
	if ( $_POST['make_it'] == 'spam' ) {
		...
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __('You are not allowed to move this item out of the Trash.', THEME_TEXT_DOMAIN) );

		if ( ! wp_untrash_post($post_id) )
			wp_die( __('Error in restoring from Trash.', THEME_TEXT_DOMAIN) );

	} elseif( $_POST['make_it'] == 'trash' ) {
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __('You are not allowed to move this item to the Trash.', THEME_TEXT_DOMAIN) );

		if ( ! wp_trash_post($post_id) )
			wp_die( __('Error in moving to Trash.', THEME_TEXT_DOMAIN) );
	...
	if ( isset( $status['publish'] ) ) {
		...
		$status_html .= '>' . __( 'Messages', THEME_TEXT_DOMAIN ) . ' <span class="count">';
	...
	if ( isset( $status['trash'] ) ) {
		...
		$status_html .= '>' . __( 'Trash', THEME_TEXT_DOMAIN ) . ' <span class="count">';
	...
	if ( isset( $status['spam'] ) ) {
		...
		$status_html .= '>' . __( 'Spam', THEME_TEXT_DOMAIN ) . ' <span class="count">';
