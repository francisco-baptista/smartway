...
# Modification of the plugin "Grunion Contact Form" for wordpress, to allow captchas, ReCaptcha.
# by marcu87 @ the google mail dot com
...
define( 'GRUNION_PLUGIN_DIR', CBP_PLUGIN_PATH . '/extensions/contact-form/' );
define( 'GRUNION_PLUGIN_URL', CBP_PLUGIN_URL . '/extensions/contact-form/');
...
function contact_form_parse( $content ) {
	...
	if ( empty($contact_form_fields) || !is_array($contact_form_fields) ) {
		...
		$default_form = '
		[contact-field label="'.__('Name', CBP_APP_TEXT_DOMAIN).'" type="name" required="true" /]
		[contact-field label="'.__('Email', CBP_APP_TEXT_DOMAIN).'" type="email" required="true" /]
		[contact-field label="'.__('Website', CBP_APP_TEXT_DOMAIN).'" type="url" /]';
		if ( 'yes' == strtolower($grunion_form->show_subject) )
			$default_form .= '
			[contact-field label="'.__('Subject', CBP_APP_TEXT_DOMAIN).'" type="subject" /]';
		$default_form .= '
		[contact-field label="'.__('Message', CBP_APP_TEXT_DOMAIN).'" type="textarea" /]';
...
function contact_form_render_field( $field ) {
	...
	$field['label'] = wp_kses( $field['label'], array() );
        ...
        // edit: removed new lines at the end of the label because wpautop made it a <br>
	if ( $field['type'] == 'email' ) {
		$r .= "\n<div>\n";
		$r .= "\t\t<label for='".esc_attr($field_id)."' class='grunion-field-label ".esc_attr($field['type']) . ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __("(mandatório)", CBP_APP_TEXT_DOMAIN) . '</span>' : '' ) . "</label>";
		$r .= "\t\t<input type='text' name='".esc_attr($field_id)."' id='".esc_attr($field_id)."' value='".esc_attr($field_value)."' class='".esc_attr($field['type'])."'/>\n";
		$r .= "\t</div>\n";
	} elseif ( $field['type'] == 'textarea' ) {
		$r .= "\n<div>\n";
		$r .= "\t\t<label for='".esc_attr($field_id)."' class='".esc_attr($field['type']) . ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __("(mandatório)", CBP_APP_TEXT_DOMAIN) . '</span>' : '' ) . "</label>";
		...
		$r .= "\t<div><label class='". ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __("(mandatório)", CBP_APP_TEXT_DOMAIN) . '</span>' : '' ) . "</label>";
		...
		$r .= "\t<div>\n";
		$r .= "\t\t<input type='checkbox' name='".esc_attr($field_id)."' value='".__('Yes', CBP_APP_TEXT_DOMAIN)."' class='".esc_attr($field['type'])."' ".( $field_value ? "checked='checked' " : "")." />\n";
		$r .= "\t\t<label class='".esc_attr($field['type']) . ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>\n";
		$r .= "\t\t". htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __("(required)", CBP_APP_TEXT_DOMAIN) . '</span>' : '' ) . "</label>";
		$r .= "\t\t<div class='clear-form'></div>\n";
		$r .= "\t</div>\n";
...
function contact_form_validate_field( $field ) {
	...
	if ( $field['required'] && $field['type'] == 'email' ) {
		if ( !is_email( $field_value ) ) {
			...
			$contact_form_errors->add( $field_id, sprintf( __( 'O campo %s não é é mandatório' , CBP_APP_TEXT_DOMAIN), $field['label'] ) );
		}
		...
		$contact_form_errors->add( $field_id, sprintf( __('O campo %s é mandatório', CBP_APP_TEXT_DOMAIN), $field['label'] ) );
	}
...
function contact_form_shortcode( $atts, $content ) {
	...
	// Recaptcha
    require_once(GRUNION_PLUGIN_DIR .'/recaptchalib.php');

    $useRecaptcha = CbpUtils::getOption('recaptcha_use');
    $publickey    = CbpUtils::getOption('rechaptcha_public_key');
    $privatekey   = CbpUtils::getOption('rechaptcha_private_key');
	...
	if ( (is_wp_error( $contact_form_errors ) && $errors = (array) $contact_form_errors->get_error_codes() )
            || (!empty($_POST) && !$resp->is_valid )
        ) {
            if ($useRecaptcha) {

                // If the recaptcha found an error:
                if (!empty($_POST) && !$resp->is_valid )
                {
                    if (!is_wp_error( $contact_form_errors ) )
                    {
                        $contact_form_errors = new WP_Error();
                    }
                    $contact_form_errors->add( "", sprintf( __( 'The Captcha is invalid, please try again', CBP_APP_TEXT_DOMAIN ) ) );
                }
            }

            if ($contact_form_errors) {

		$r .= "<div class='form-error'>\n<h3>" . __( 'Erro!', CBP_APP_TEXT_DOMAIN ) . "</h3>\n<ul class='form-errors'>\n";
		foreach ( $contact_form_errors->get_error_messages() as $message )
			$r .= "\t<li class='form-error-message' style=''>$message</li>\n";
		$r .= "</ul>\n</div>\n\n";
            }
	}
	...
	$r .= "\t<p class='contact-submit'>\n";
        ...
        if ($useRecaptcha) {
            $r .= "\t". recaptcha_get_html($publickey) ."\n\n<br>";
        }

	$r .= "\t\t<input type='submit' value='" . __( "Enviar" , CBP_APP_TEXT_DOMAIN) . "' class='pushbutton-wide btn'/>\n";
	$r .= "\t\t$nonce\n";
	...
	$errors = array();
	if ( is_wp_error( $contact_form_errors ) && $errors = (array) $contact_form_errors->get_error_codes() ) :
		$r .= "<div class='form-error'>\n<h3>" . __( 'Error!' , CBP_APP_TEXT_DOMAIN) . "</h3>\n<p>\n";
		foreach ( $contact_form_errors->get_error_messages() as $message )
			$r .= "\t$message<br />\n";
		$r .= "</p>\n</div>\n\n";
	else :
		$r .= "<h3>" . __( 'Message Sent', CBP_APP_TEXT_DOMAIN) . "</h3>\n\n";
...
*/
	...
	$time = date_i18n( __('l F j, Y \a\t g:i a', CBP_APP_TEXT_DOMAIN), current_time( 'timestamp' ) );
...
" . __( "Time:" , CBP_APP_TEXT_DOMAIN) . " " . $time . "
" . __( "IP Address:", CBP_APP_TEXT_DOMAIN ) . " " . $comment_author_IP . "
" . __( "Contact Form URL:", CBP_APP_TEXT_DOMAIN ) . " " . get_permalink( $post->ID ) . "
...
";
	...
	if ( is_user_logged_in() ) {
		$message .= sprintf(
			__( "\nSent by a verified %s user." , CBP_APP_TEXT_DOMAIN),
			isset( $GLOBALS['current_site']->site_name ) && $GLOBALS['current_site']->site_name ? $GLOBALS['current_site']->site_name : '"' . get_option( 'blogname' ) . '"'
		);
	} else {
		$message .= __( "Sent by an unverified visitor to your site." , CBP_APP_TEXT_DOMAIN);
	}
...
function contact_form_init() {
	...
	register_post_type( 'feedback', array(
		'labels'	=> array(
			'name'			=> __( 'Form Feedbacks', CBP_APP_TEXT_DOMAIN ),
			'singular_name'	=> __( 'Feedback', CBP_APP_TEXT_DOMAIN ),
			'search_items'	=> __( 'Search Feedback', CBP_APP_TEXT_DOMAIN ),
			'not_found'		=> __( 'No feedback found', CBP_APP_TEXT_DOMAIN ),
			'not_found_in_trash'	=> __( 'No feedback found', CBP_APP_TEXT_DOMAIN )
		),
...
function grunion_media_button( ) {
	...
	$title = esc_attr( __( 'Add a custom form', CBP_APP_TEXT_DOMAIN ) );
